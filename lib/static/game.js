...
let wim = innerHeight -innerWidth
console.log(wim)
...
let wordContainerWidth
let wordContainerHeight

let submenuX
let submenuY

function calculatedContainers() {
  if (wim <= 1) {
    ...
    submenuX = 0
    submenuY = 0
    // gameContainerHeight + wordContainerHeight + yhGap
  }
  return
}
...
const bgColorText = 'rgba(5, 5, 30, 0)'
...
const startButton = d3.select('#saga').append('g')
  .attr('id', 'saga-button')
...
const skyliutysOffset = buttonRadius /5
  ...
  startButton.append('text')
    .append("textPath")
    .attr("xlink:href", "#arcUp")
    .attr('class', 'start-button-text')
    .attr('x', -buttonRadius / 12)
    .attr('y', -buttonRadius / 2)
    .attr("startOffset", "50%")
    .text('START BUTTON')
    .style('font-size', `${buttonRadius / 4}px`)

  startButton.append('text')
    .append("textPath")
    .attr("xlink:href", "#arcDown")
    .attr('class', 'start-button-text')
...
function checkIfDone() {
  if (rightLetters.length === randomWord.length) {
   ...
   Promise.all(pusher).then(randomBalls())
   // does the job, but with errors
...
this.keyboardKey =function (click) {
