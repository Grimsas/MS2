...
let GCH20 = parseInt(gameContainerHeight / 20)
...
function ball (letter) {
    this.cx = rndPos(xwGap + GCH20, gameContainerWidth - GCH20*2)
    this.cy = rndPos(yhGap + GCH20, gameContainerHeight - GCH20*2 - 10)
    this.letter = letter
    ...
    this.draw = function () {
      let ball = ballsGroup.selectAll('#' + this.letter)
        .data([null])
      ball.enter()
        .append('circle')
        .attr('id', this.letter)
      ...
      // border detection
      this.move = function () {
      if (this.cx + GCH20 + this.dx> parseInt(gameContainerWidth- GCH20 ) || this.cx + GCH20 + this.dx < GCH20) {
        this.dx = -this.dx
      }
      if (this.cy + GCH20 + this.dy > parseInt(gameContainerHeight - yhGap ) - GCH20 || this.cy + GCH20 + this.dy < GCH20) {
        this.dy = -this.dy
      }
      //movement
      this.cx += this.dx
