...
let commonWords =  d3.json("../../srv/js/common.json").then(function(data) {
  commonWords = data.commonWords
   }
)

//map dictionary to arrays by length
function mapDictionaryToLenght() {

for (let i=0; i<commonWords.length; i++)
    console.log(i + commonWords[i].length);
}
...
// redraw on screen size change

/////////////////////////////////////////////////////////Game Window//////////////////////////////////////////////////////////////////////////

// Generate Window
let svg = d3.select('.game-background')
  .append('svg')
    .attr('width', WIW)
    .attr('height', WIH)
    .attr("opacity","0.9")
...
let verticalGap = parseInt(WIW * 0.01)
let horizontalGap = parseInt(WIH * 0.01)
let randomColor = ballColors['i']  //not random anymore, let name to be changed

let gameContainer = svg.append('rect')
  .attr('x', verticalGap)
  .attr('y', horizontalGap)
...
// create menu layout
let menuContainerWidth = WIW - (verticalGap * 3 + gameContainerWidth)
let menuContainerHeight = WIH - (horizontalGap * 2)

let menuContainer = svg.append('rect')
  .attr('x', verticalGap * 2 + gameContainerWidth)
  .attr('y', horizontalGap)
  .attr('width', menuContainerWidth)
  .attr('height', menuContainerHeight - horizontalGap)
  .attr('fill', randomColor)
  .attr('stroke', randomColor)

 // create words container   
let wordContainerWidth = WIW - (verticalGap * 3 + menuContainerWidth)
let wordContainerHeight = WIH - (horizontalGap * 3 + gameContainerHeight)

let wordContainer = svg.append('rect')
  .attr('x', verticalGap)
  .attr('y', horizontalGap * 2  + gameContainerHeight)
  .attr('width', wordContainerWidth)
  .attr('height', wordContainerHeight - horizontalGap)
  .attr('fill', randomColor)
  .attr('stroke', randomColor)
...
function pickRandom(a) {
  ...
  let rnd = Math.trunc(prop.length * Math.random())    // 5 = from  26 * 0.1923076923076923 to 26 * 0.2307692307692307 (letter chance ~3.84%)
  return {
...
const difficultySelection = {
  easiest: {lives: 5, additionalBalls: 5, time: 120, scoreMultiplier: 1, wordLength:5},
  easy: {lives: 4, additionalBalls: 6, time: 120, scoreMultiplier: 1.25, wordLength:6},
  medium: {lives: 3, additionalBalls: 7, time: 120, scoreMultiplier: 1.5, wordLength:7},
  hard: {lives: 2, additionalBalls: 8, time: 120, scoreMultiplier: 1.75, wordLength:8},
  hardest: {lives: 1, additionalBalls: 9, time: 120, scoreMultiplier: 2, wordLength:9}
}
...
let startingWordLength = difficulty.wordLength
