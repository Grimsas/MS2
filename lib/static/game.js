...
let wordFieldOutG = wordContainerG.append('g')
  .attr('id', 'wordToSolve')
  .attr("transform", `translate(${(wordContainerWidth /2)}, ${yhGap})`)
let wordFieldOut = wordFieldOutG.append('rect')
  .attr('width', randomWord.length * 50)
  .attr('height', wordContainerHeight - yhGap * 3)
  .attr('fill', 'white')
...
const letterWidth = parseInt((wordContainerWidth * 0.075))
...
function Word() {
  ...
  gEnter
    .merge(g)
    .attr('transform', (d, i) => `translate(${(i * letterWidth) -randomWord.length*50},${parseInt(yOffset / 2)})`)
  g.exit().remove()
  ...
  for (element in balls) {
    if (balls[element].letter == letter) {
      ...
      d3.selectAll(`#${ltr}`).select(`#${ltr}-ball`)
        ...
        .transition().ease(d3.easeExpInOut).duration(1000).attr('r', 0)
      d3.selectAll(`#${ltr}`).select(`#${ltr}-letter`)
        .transition().style('font-size', '72px').attr('y', 20)
        .transition().ease(d3.easeExpInOut).duration(1000).attr('y', 0).style('font-size', '0px')
      d3.selectAll(`#${ltr}`).transition().duration(5200).remove()

      d3.selectAll('#wordToSolve').selectAll(`#${ltr}-square`)
        .transition().ease(d3.easeExpInOut).duration(1000).attr('fill', 'black')
      d3.selectAll('#wordToSolve').selectAll(`#${ltr}-toGuess`)
        ...
        .transition().ease(d3.easeExpInOut).style('font-size', '4em')
    }
  }
}
...
// Known Bug. Doesn't work on mobile. dictionaryA() invocation state change handling too early or something
function rndWord() {
