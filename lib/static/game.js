...
let WIHM = innerWidth
let WIWM = innerHeight

let balls = []
let rightLetters = []

let TimeElapsed = 0
let TimeElapsedFlag = 0
let TimeElapsedID
let wordCount = -1
let letterCount = 0
let mistakes = 0
let score = 0
let ballSpeed = 2 * difficulty.scoreMultiplier + (WIH / 1500) + (WIW / 1500)
let ballSize = 25 + (WIH / 50) + (WIW / 50)
let timeLeft
function timeCalc(){let time = startingTime - TimeElapsed
timeLeft = time}


const letterPointsValue = 10
const wordPointsValue = 50
score -= wordPointsValue * difficulty.scoreMultiplier //score start from ZERO.
...
const startButton = d3.select('#saga')
...
const skyliutysOffset = WIH/15
const skyliutysTextOffset = WIH/7
const textOffsetY = WIH/2+WIH/4/6
  const skyliutys = d3.select('#sagute')
  startButton.append('circle')
  .attr('cx', WIW/2+ skyliutysOffset)
  .attr('cy', WIH/2+ skyliutysOffset)
  .attr('r', WIH/4/6)
  .attr('fill', '#444')
  startButton.append('circle')
  .attr('cx', WIW/2 +skyliutysOffset)
  ...
  .attr('id', 'Start-text')
  .attr('x', WIW/2-skyliutysTextOffset)
  .attr('y', WIH/2-skyliutysTextOffset)
  .text('Start Button')
  .style('font-size', '3em')
  ...
  startButton.append('text')
  .attr('id', 'Start-text')
  .attr('x', WIW/2-skyliutysTextOffset-WIH/20)
  .attr('y', WIH/2+skyliutysTextOffset+WIH/40)
...
function ScoreCalc() {
  ...
  const fieldWidth = parseInt(menuContainerWidth - xwGap * 2)
  ...
  gEnter.append('text')
    ...
    .attr('id', 'scoretet')
    .attr('x', -fieldWidth / 2)
    .attr('y',menuContainerHeight / 10 +  yhGap * 6)
    .text('Score: ')
    .style('fill', 'white')
    .style('font-weight', 'bold')
    .style('font-size', '2em')
  ...
  gEnter.append('text')
    ...
    .attr('id', 'scorenum')
    .attr('x', fontXoffset)
    .attr('y',  menuContainerHeight / 10 + yhGap * 11)
    .text(score)
    .style('fill', 'white')
    .style('font-weight', 'bold')
    .style('font-size', '2em')
...
const progressBar = wordContainerG.append('rect')
 ...
 const progress = wordContainerG.append('rect')
   .attr('fill', 'red')
   .attr('y', -yhGap/3)
...
function runningOut(){
  progress.attr('fill', 'red').attr('width', wordContainerWidth / startingTime)
  .attr('x', second*TimeElapsed)
}
  ...
  constructor(letter) {
    ...
    this.draw = function () {
      ...
      ballGenter.append('circle')
        ...
        .attr('class', 'ball')
        .transition().attr('r', ballSize + ballSize / 3)
      ...
      ballGenter.append('text')
        ...
        .text(this.letter)
        // .transition().ease(d3.easeExpInOut).style('font-size', '0.1em')
        .transition().style('font-size', '5em')
        .attr('y', 20)
    }
  ...
  for (element in balls) {
    if (balls[element].letter == letter && randomWord.some(value => value == ltr)) {
      ...
      d3.selectAll(`#${ltr}`).select(`#${ltr}-letter`)
        .attr('id', null)
        .transition().style('font-size', '7em').attr('y', ballSize/2)
      ...
      d3.selectAll('#wordToSolve').select(`#${ltr}-toGuess`)
        .attr('id', null)
        .transition().style('font-size', '1.0em')
      ...
      d3.selectAll(`#${ltr}`).select(`#${ltr}-ball`)
        ...
        .transition().ease(d3.easeExpInOut).duration(500).attr('r', ballSize)
      d3.selectAll(`#${ltr}`).select(`#${ltr}-letter`)
        .transition().ease(d3.easeExpIn).duration(150).attr('y', 20).style('font-size', '5em')
        .transition().ease(d3.easeExpInOut).style('font-size', '5em')
...
function timer() {
  ...
  TimeElapsedID = setTimeout(function () {
    ...
    if (timeLeft === 0){
      ...
      // You completed ${wordCount} words,
      // You clicked ${letterCount} letters,
      // and you made just ${mistakes} mistakes!`)
    }
}, 1000)
}
